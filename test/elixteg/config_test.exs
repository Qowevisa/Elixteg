defmodule Elixteg.ConfigTest do
  use ExUnit.Case
  alias Elixteg.Config

  defp restore_env!(key, value) do
    if value do
      :ok = Application.put_env(:elixteg, key, value)
    else
      :ok = Application.delete_env(:elixteg, key)
    end
  end

  setup do
    base_url = Application.get_env(:elixteg, :base_url)
    graph_base_url = Application.get_env(:elixteg, :graph_base_url)
    file_base_url = Application.get_env(:elixteg, :file_base_url)
  ...
  test "Config.file_base_url/0 returns default when unset" do
    :ok = Application.delete_env(:elixteg, :file_base_url)
