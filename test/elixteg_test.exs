defmodule ElixtegTest do
  use ExUnit.Case, async: false
  use ExVCR.Mock, adapter: ExVCR.Adapter.Hackney
  doctest Elixteg, only: [get_file_link: 1]
  alias Elixteg.Model.User

  setup_all do
    unless Application.get_env(:elixteg, :token) do
      Application.put_env(:elixteg, :token, {:system, "ENV_TOKEN", "TEST_TOKEN"})
    end
  ...
  test "get_me" do
    use_cassette "get_me" do
      {:ok, me} = Elixteg.get_me()
      assert me == %User{id: 666, first_name: "Elixteg", username: "elixteg_bot"}
    end
  end

  test "send_message" do
    use_cassette "send_message" do
      {:ok, message} = Elixteg.send_message(666, "aloha")
  ...
  test "forward_message" do
    use_cassette "forward_message" do
      {:ok, message} = Elixteg.forward_message(666, 666, 666)
  ...
  test "send_photo" do
    use_cassette "send_photo" do
      ...
      {:ok, message} = Elixteg.send_photo(666, file_id)
  ...
  test "send_sticker" do
    use_cassette "send_sticker" do
      {:ok, message} = Elixteg.send_sticker(666, "BQADBQADBgADmEjsA1aqdSxtzvvVAg")
  ...
  test "send_contact" do
    use_cassette "send_contact" do
      {:ok, message} = Elixteg.send_contact(666, 10_123_800_555, "Test")
  ...
  test "send_location" do
    use_cassette "send_location" do
      {:ok, message} = Elixteg.send_location(666, 1, 2)
  ...
  test "send_venue" do
    use_cassette "send_venue" do
      {:ok, message} = Elixteg.send_venue(666, 1, 2, "Test", "teststreet")
  ...
  test "delete_message" do
    use_cassette "delete_message" do
      assert Elixteg.delete_message(1, 666) == :ok
    end
  end

  test "send_chat_action" do
    use_cassette "send_chat_action" do
      assert Elixteg.send_chat_action(666, "typing") == :ok
    end
  end

  test "get_user_profile_photos" do
    use_cassette "get_user_profile_photos" do
      {:ok, user_profile_photos} = Elixteg.get_user_profile_photos(666)
  ...
  test "get_updates" do
    use_cassette "get_updates" do
      {:ok, updates} = Elixteg.get_updates(limit: 1)
  ...
  test "set webhook" do
    use_cassette "set_webhook" do
      assert Elixteg.set_webhook(url: "https://telegram.org/") == :ok
    end
  end

  test "get webhook info" do
    use_cassette "get_webhook_info" do
      webhook_info = %Elixteg.Model.WebhookInfo{
        allowed_updates: [],
      ...
      assert Elixteg.get_webhook_info() == {:ok, webhook_info}
    end
  end

  test "delete webhook" do
    use_cassette "delete_webhook" do
      assert Elixteg.delete_webhook() == :ok
    end
  end

  test "get_file" do
    use_cassette "get_file" do
      {:ok, file} = Elixteg.get_file("BQADBQADBgADmEjsA1aqdSxtzvvVAg")
  ...
  test "get_chat" do
    use_cassette "get_chat" do
      {:ok, chat} = Elixteg.get_chat("@group")
  ...
  test "get_chat_member" do
    use_cassette "get_chat_member" do
      {:ok, chat_member} = Elixteg.get_chat_member("@group", 666)
      assert chat_member.user.username == "elixteg_bot"
  ...
  test "get_chat_administrators" do
    use_cassette "get_chat_administrators" do
      {:ok, [admin | [creator]]} = Elixteg.get_chat_administrators("@group")
      assert admin.status == "administrator"
      assert admin.user.username == "elixteg_bot"
  ...
  test "get_chat_members_count" do
    use_cassette "get_chat_members_count" do
      {:ok, count} = Elixteg.get_chat_members_count("@group")
      assert count == 2
    end
  end
  ...
  test "leave_chat" do
    use_cassette "leave_chat" do
      assert Elixteg.leave_chat("@group") == :ok
    end
  end

  test "answer_inline_query" do
    photo = %Elixteg.Model.InlineQueryResult.Photo{
      id: "1",
      photo_url:
    ...
    use_cassette "answer_inline_query" do
      assert :ok == Elixteg.answer_inline_query(666, [photo])
    end
  end

  test "get_sticker_set" do
    use_cassette "get_sticker_set" do
      {:ok, sticker_set} = Elixteg.get_sticker_set("TomNJerry")
  ...
  test "upload_sticker_file" do
    use_cassette "upload_sticker_file" do
      {:ok, file} = Elixteg.upload_sticker_file(666, "BQADBAADzwADZgctUjisl0we_2qGAg")
  ...
  test "create_new_sticker_set" do
    use_cassette "create_new_sticker_set" do
      assert :ok =
               Elixteg.create_new_sticker_set(
                 666,
                 "test_sticker_set_by_elixteg_bot",
                 "elixteg test",
  ...
  test "set_sticker_position_in_set" do
    use_cassette "set_sticker_position_in_set" do
      assert :ok = Elixteg.set_sticker_position_in_set("CAADBQADLgADmEjsA7jm5QOy8WxsAg", 0)
    end
  end

  test "delete_sticker_from_set" do
    use_cassette "delete_sticker_from_set" do
      assert :ok = Elixteg.delete_sticker_from_set("CAADBQADLgADmEjsA7jm5QOy8WxsAg")
    end
  end
  ...
  test "send_animation" do
    use_cassette "send_animation" do
      ...
      {:ok, message} = Elixteg.send_animation(666, file_id)
  ...
  test "pin_chat_message" do
    use_cassette "pin_chat_message" do
      assert :ok = Elixteg.pin_chat_message(666, 666)
    end
  end

  test "unpin_chat_message" do
    use_cassette "unpin_chat_message" do
      assert :ok = Elixteg.unpin_chat_message(666)
    end
  end
end
